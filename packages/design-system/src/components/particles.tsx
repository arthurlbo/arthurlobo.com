...
interface IParticlesProps extends ComponentPropsWithoutRef<"div"> {
    className?: string;
    quantity?: number;
    staticity?: number;
    ease?: number;
...
export const Particles: React.FC<IParticlesProps> = ({
    ...
    const drawCircle = (circle: TCircle, update = false) => {
        if (context.current) {
    ...
    const animate = () => {
        clearContext();
        circles.current.forEach((circle: TCircle, i: number) => {
