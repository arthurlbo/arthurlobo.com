...
export const Card = ({ children, link, className }: IProps) => {
    return (
        <Link
            href={link}
            className={cn(
                "border-surface-500 group relative flex h-[200px] w-full cursor-pointer items-start justify-between gap-4 overflow-hidden rounded-2xl border transition-all duration-300 ease-in-out lg:h-full",
                className,
            )}
        >
            {children}
